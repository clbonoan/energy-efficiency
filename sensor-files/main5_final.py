...
def update_lux():
    # this accounts for the time.sleep necessary for the light sensor to 
    # properly read data without interfering with the timely updates
    # needed with the mmWave sensor
    global lux
    while True:
        lux = read_lux()
        time.sleep(0.15)    # required

# start background thread for lux reading
lux_thread = threading.Thread(target=update_lux, daemon = True)
lux_thread.start()
...
while True:
    ...
    if light_on and distance_cm > DISTANCE_THRESHOLD:
        #lux = read_lux()
        brightness = calculate_brightness(lux)
        print(f"Brightness check: {brightness} (lux = {lux:.2f})")
        if brightness > 0:
